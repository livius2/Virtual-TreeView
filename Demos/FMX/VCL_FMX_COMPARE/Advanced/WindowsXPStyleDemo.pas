...
uses
  ...
  Dialogs, VirtualTrees, ImgList, ComCtrls, ToolWin, Menus, StdCtrls, UITypes, System.ImageList;
...
begin
  XPTree:= TVirtualStringTree.Create(Self);
  XPTree.Parent:= Self;
  XPTree.NodeDataSize := SizeOf(TEntry);

  XPTree.OnCompareNodes := XPTreeCompareNodes;
  XPTree.OnFreeNode := XPTreeFreeNode;
  XPTree.OnGetText := XPTreeGetText;
  XPTree.OnGetImageIndex := XPTreeGetImageIndex;
  XPTree.OnGetHint := XPTreeGetHint;
  XPTree.OnHeaderClick := XPTreeHeaderClick;
  XPTree.OnInitChildren := XPTreeInitChildren;
  XPTree.OnInitNode := XPTreeInitNode;
  XPTree.OnStateChange := XPTreeStateChange;
  ...
  XPTree.Height := 447;
  XPTree.Align := alLeft;
  XPTree.Anchors := [akLeft, akTop, akRight, akBottom];
  XPTree.AutoScrollDelay := 500;
  XPTree.BevelInner := bvNone;
  XPTree.ButtonFillMode := fmShaded;
  {XPTree.ClipboardFormats.Strings := (
    'CSV'
    'HTML Format')}
  XPTree.Colors.DropTargetColor := 7063465;
  XPTree.Colors.DropTargetBorderColor := 4958089;
  XPTree.Colors.FocusedSelectionColor := clActiveCaption;
  XPTree.Colors.FocusedSelectionBorderColor := clActiveCaption;
  XPTree.Colors.GridLineColor := clBtnShadow;
  XPTree.Colors.UnfocusedSelectionBorderColor := clBtnShadow;
  XPTree.DefaultNodeHeight := 34;
  XPTree.DragMode := dmAutomatic;
  XPTree.DrawSelectionMode := smBlendedRectangle;
  XPTree.Header.AutoSizeIndex := -1;
  XPTree.Header.Font.Charset := ANSI_CHARSET;
  XPTree.Header.Font.Color := clWindowText;
  XPTree.Header.Font.Height := -12;
  XPTree.Header.Font.Name := 'Microsoft Sans Serif';
  XPTree.Header.Font.Style := [];
  XPTree.Header.Height := 24;
  XPTree.Header.Options := [hoColumnResize, hoDrag, hoHotTrack, hoShowSortGlyphs, hoVisible];
  //XPTree.HintAnimation := hatFade
  XPTree.HintMode := hmHint;
  XPTree.Images := LargeImages;
  XPTree.ParentShowHint := False;
  XPTree.RootNodeCount := 18;
  XPTree.ShowHint := True;
  XPTree.StateImages := SmallImages;
  XPTree.TabOrder := 0;
  XPTree.TreeOptions.AnimationOptions := [toAnimatedToggle];
  XPTree.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoScrollOnExpand, toAutoTristateTracking];
  XPTree.TreeOptions.MiscOptions := [toAcceptOLEDrop, toCheckSupport, toInitOnSave, toReportMode, toToggleOnDblClick, toWheelPanning];
  XPTree.TreeOptions.PaintOptions := [toHideFocusRect, toHotTrack, toShowButtons, toShowDropmark, toShowRoot, toThemeAware, toUseExplorerTheme];
  XPTree.TreeOptions.SelectionOptions := [toFullRowSelect, toMultiSelect, toSimpleDrawSelection];

  col:= XPTree.Header.Columns.Add;
  col.Options := [coAllowClick, coEnabled, coParentBidiMode, coParentColor, coResizable, coShowDropMark, coVisible];
  col.Position := 0;
  col.Width := 200;
  col.Text := 'Name';
