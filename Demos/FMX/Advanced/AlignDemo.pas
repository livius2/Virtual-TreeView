...
begin
  AlignTree:= TVirtualStringTree.Create(Self);
  AlignTree.OnFocusChanged := AlignTreeFocusChanged;
  AlignTree.OnFreeNode := AlignTreeFreeNode;
  AlignTree.OnGetText := AlignTreeGetText;
  AlignTree.OnPaintText := AlignTreePaintText;
  AlignTree.OnGetImageIndex := AlignTreeGetImageIndex;
  AlignTree.OnGetNodeDataSize := AlignTreeGetNodeDataSize;
  AlignTree.OnHeaderClick := AlignTreeHeaderClick;
  AlignTree.OnInitChildren := AlignTreeInitChildren;
  AlignTree.OnInitNode := AlignTreeInitNode;
  AlignTree.OnStateChange := AlignTreeStateChange;
  AlignTree.Images := TreeImages;
  AlignTree.StateImages := TreeImages;
  AlignTree.Parent:= Self;
  AlignTree.Left := 220;
  AlignTree.Top := 2;
  AlignTree.Width := 567;
  AlignTree.Height := 335;
  AlignTree.Anchors := [akLeft, akTop, akRight, akBottom];
  AlignTree.BevelOuter := bvNone;
  {AlignTree.ClipboardFormats.Strings := (
    'CSV'
    'HTML Format'
    'Plain text'
    'Rich Text Format'
    ...
    'Unicode text'
    'Virtual Tree Data')}
  AlignTree.Colors.BorderColor := clBackground;
  AlignTree.Colors.FocusedSelectionColor := clSilver;
  AlignTree.Colors.FocusedSelectionBorderColor := 10526880;
  AlignTree.Colors.HotColor := clBlack;
  AlignTree.Colors.UnfocusedSelectionBorderColor := clBtnShadow;
  AlignTree.DefaultNodeHeight := 34;
  AlignTree.DragMode := dmAutomatic;
  AlignTree.EditDelay := 100;
  AlignTree.Header.AutoSizeIndex := 0;
  AlignTree.Header.Background := clBtnShadow;
  AlignTree.Header.Font.Charset := DEFAULT_CHARSET;
  AlignTree.Header.Font.Color := clWindowText;
  AlignTree.Header.Font.Height := -11;
  AlignTree.Header.Font.Name := 'Tahoma';
  AlignTree.Header.Font.Style := [];
  AlignTree.Header.Height := 40;
  AlignTree.Header.Images := HeaderImages;
  AlignTree.Header.Options := [hoColumnResize, hoDblClickResize, hoDrag, hoShowHint, hoShowImages, hoShowSortGlyphs, hoVisible, hoAutoSpring];
  AlignTree.Header.PopupMenu := IconPopup;
  AlignTree.Header.Style := hsFlatButtons;
  AlignTree.HintMode := hmTooltip;
  AlignTree.HotCursor := crHandPoint;

  AlignTree.ParentShowHint := False;
  AlignTree.RootNodeCount := 6;
  AlignTree.SelectionCurveRadius := 5;
  AlignTree.ShowHint := True;
  AlignTree.TabOrder := 0;
  AlignTree.TreeOptions.AnimationOptions := [toAnimatedToggle];
  AlignTree.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoTristateTracking, toAutoDeleteMovedNodes];
  AlignTree.TreeOptions.MiscOptions := [toAcceptOLEDrop, toCheckSupport, toEditable, toFullRepaintOnResize, toInitOnSave, toToggleOnDblClick, toWheelPanning];
  AlignTree.TreeOptions.PaintOptions := [toHideFocusRect, toShowButtons, toShowDropmark, toShowRoot, toShowTreeLines, toThemeAware];
  AlignTree.TreeOptions.SelectionOptions := [toExtendedFocus, toMultiSelect];

  col:= AlignTree.Header.Columns.Add;
  col.Alignment := taCenter;
  col.BiDiMode := bdLeftToRight;
  col.Color := clWindow;
  col.ImageIndex := 20;
  col.Options := [coAllowClick, coDraggable, coEnabled, coResizable, coShowDropMark, coVisible, coAutoSpring, coWrapCaption];
  col.Position := 0;
  col.Width := 189;
  col.Text := 'English text column';
  col.Hint :=
  ...
  col.Position := 1;
  col.Width := 159;
  col.Text := 'Greek text column';
  col.Hint :=
        'Text is initially left aligned and has a left-to-right direction' +
        'ality.';

  col:= AlignTree.Header.Columns.Add;
  col.BiDiMode := bdRightToLeft;
  col.Color := clWindow;
  col.ImageIndex := 32;
  col.Options := [coAllowClick, coDraggable, coEnabled, coResizable, coShowDropMark, coVisible, coAutoSpring, coWrapCaption];
  col.Position := 2;
  col.Width := 213;
  col.Text := 'Hebrew/arabic text column';
  col.Hint :=
