...
uses
  ...
  StdActns, ActnList, VirtualTrees.HeaderPopup, UITypes, VTHeaderPopup, System.ImageList;
...
begin
  ...
  VST2.Parent:= Self;
  VST2.OnDragOver := VST2DragOver;
  VST2.OnFocusChanging := VST2FocusChanging;
  VST2.OnFreeNode := VST2FreeNode;
  VST2.OnGetText := VST2GetText;
  VST2.OnPaintText := VST2PaintText;
  VST2.OnGetImageIndexEx := VST2GetImageIndexEx;
  VST2.OnGetNodeDataSize := VST2GetNodeDataSize;
  VST2.OnGetPopupMenu := VST2GetPopupMenu;
  VST2.OnInitChildren := VST2InitChildren;
  VST2.OnInitNode := VST2InitNode;
  VST2.OnKeyDown := VST2KeyDown;
  VST2.OnNewText := VST2NewText;
  VST2.OnStateChange := VST2StateChange;

  VST2.ScrollBarOptions.VerticalIncrement := 19;
  VST2.TabOrder := 0;
  VST2.TreeOptions.AnimationOptions := [toAnimatedToggle];
  VST2.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoSpanColumns, toAutoTristateTracking, toAutoHideButtons, toDisableAutoscrollOnFocus, toAutoChangeScale];
  VST2.TreeOptions.MiscOptions := [toAcceptOLEDrop, toCheckSupport, toEditable, toInitOnSave, toToggleOnDblClick, toWheelPanning, toEditOnClick];
  VST2.TreeOptions.PaintOptions := [toHideSelection, toHotTrack, toShowButtons, toShowDropmark, toShowRoot, toShowTreeLines, toShowVertGridLines, toThemeAware, toUseBlendedImages, toFullVertGridLines, toUseBlendedSelection];
  VST2.TreeOptions.SelectionOptions := [toExtendedFocus, toMiddleClickSelect, toMultiSelect, toRightClickSelect];
  VST2.TreeOptions.StringOptions := [toSaveCaptions, toShowStaticText, toAutoAcceptEditChange];
  ...
  VST2.Height := 497;
  VST2.Align := alClient;
  VST2.AutoExpandDelay := 300;
  VST2.BiDiMode := bdLeftToRight;
  VST2.ButtonFillMode := fmWindowColor;
 { VST2.ClipboardFormats.Strings := (
    'HTML Format'
    'Unicode text'
    'Virtual Tree Data')}
  VST2.Colors.BorderColor := clScrollBar;
  VST2.Colors.DropTargetColor := clActiveBorder;
  VST2.Colors.DropTargetBorderColor := clActiveCaption;
  VST2.Colors.HotColor := clBlack;
  VST2.Colors.UnfocusedSelectionBorderColor := clBtnShadow;
  VST2.Ctl3D := True;
  VST2.CustomCheckImages := TreeImages;
  VST2.DefaultNodeHeight := 20;
  VST2.DragCursor := crHelp;
  VST2.DragMode := dmAutomatic;
  VST2.DrawSelectionMode := smBlendedRectangle;
  VST2.EditDelay := 200;
  VST2.Font.Charset := ANSI_CHARSET;
  VST2.Font.Color := clWindowText;
  VST2.Font.Height := -11;
  VST2.Font.Name := 'Lucida Sans Unicode';
  VST2.Font.Style := [];
  VST2.Header.AutoSizeIndex := -1;
  VST2.Header.Background := clBtnShadow;
  ...
  VST2.Header.Options := [hoColumnResize, hoDblClickResize, hoDrag, hoHotTrack, hoShowHint, hoVisible];
  VST2.Header.PopupMenu := VTHPopup;
  VST2.Header.Style := hsFlatButtons;
  VST2.HotCursor := crHandPoint;
  VST2.Images := TreeImages;
  VST2.ParentBiDiMode := False;
  VST2.ParentCtl3D := False;
  VST2.ParentFont := False;

  col:= VST2.Header.Columns.Add;
  col.ImageIndex := 10;
  col.Options := [coDraggable, coEnabled, coParentBidiMode, coParentColor, coResizable, coShowDropMark, coVisible];
  col.Position := 1;
  col.Width := 300;
  col.Text := 'Initial main column';
  col.Hint := 'Column which initially contains the tree.';

  col:= VST2.Header.Columns.Add;
  col.Color := 14737632;
  col.MaxWidth := 22;
  col.Options := [coEnabled, coParentBidiMode, coVisible];
  col.Position := 0;
  col.Spacing := 0;
  col.Width := 22;
  col.Hint := 'Image only column.';

  col:= VST2.Header.Columns.Add;
  col.BiDiMode := bdLeftToRight;
  col.ImageIndex := 4;
  col.Options := [coAllowClick, coEnabled, coParentColor, coResizable, coShowDropMark, coVisible];
  col.Position := 2;
  col.Width := 300;
  col.Text := 'Language column';
  col.Hint :=
