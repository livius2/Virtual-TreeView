...
begin
  VDT1:= TVirtualDrawTree.Create(Self);
  VDT1.Parent:= Self;
  VDT1.OnCompareNodes := VDT1CompareNodes;
  VDT1.OnDrawHint := VDT1DrawHint;
  VDT1.OnDrawNode := VDT1DrawNode;
  VDT1.OnFreeNode := VDT1FreeNode;
  VDT1.OnGetHintSize := VDT1GetHintSize;
  VDT1.OnGetImageIndex := VDT1GetImageIndex;
  VDT1.OnGetNodeWidth := VDT1GetNodeWidth;
  VDT1.OnHeaderClick := VDT1HeaderClick;
  VDT1.OnInitChildren := VDT1InitChildren;
  VDT1.OnInitNode := VDT1InitNode;
  VDT1.OnStateChange := VDT1StateChange;
  ...
  VDT1.Width := 684;
  VDT1.Height := 278;
  VDT1.Anchors := [akLeft, akTop, akRight, akBottom];
  VDT1.AutoExpandDelay := 200;
  VDT1.AutoScrollDelay := 200;
  VDT1.BevelInner := bvNone;
  VDT1.BevelKind := bkTile;
  VDT1.BorderStyle := bsNone;
  {VDT1.ClipboardFormats.Strings := (
    'Virtual Tree Data'); }
  VDT1.Colors.BorderColor := clWindowText;
  VDT1.Colors.HotColor := clBlack;
  VDT1.Colors.TreeLineColor := clBtnFace;
  VDT1.DefaultNodeHeight := 32;
  VDT1.Header.AutoSizeIndex := -1;
  VDT1.Header.Background := clBtnHighlight;
  VDT1.Header.Height := 22;
  VDT1.Header.Options := [hoColumnResize, hoDblClickResize, hoDrag, hoRestrictDrag, hoShowSortGlyphs, hoVisible];
  VDT1.Header.ParentFont := True;
  VDT1.Header.Style := hsPlates;
  //VDT1.HintAnimation := hatNone;
  VDT1.HintMode := hmHint;
  VDT1.Images := SystemImages;
  VDT1.IncrementalSearch := isAll;
  VDT1.Indent := 20;
  VDT1.LineMode := lmBands;
  VDT1.ParentShowHint := False;
  VDT1.RootNodeCount := 10;
  VDT1.ScrollBarOptions.VerticalIncrement := 32;
  VDT1.ShowHint := True;
  VDT1.TabOrder := 0;
  VDT1.TreeOptions.AnimationOptions := [toAnimatedToggle];
  VDT1.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoScrollOnExpand, toAutoTristateTracking, toAutoDeleteMovedNodes];
  VDT1.TreeOptions.PaintOptions := [toShowBackground, toShowButtons, toShowDropmark, toShowHorzGridLines, toShowRoot, toShowTreeLines, toShowVertGridLines, toThemeAware];
  VDT1.TreeOptions.SelectionOptions := [toFullRowSelect];
  col:= VDT1.Header.Columns.Add;
          ...
          begin
            ...
            begin
              if (NodeWidth - 2 * Margin) > (Right - Left) then
                S := ShortenString(Canvas, S, Right - Left);
            end;
