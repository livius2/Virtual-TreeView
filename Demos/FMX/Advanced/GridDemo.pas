...
begin
  VST5:= TVirtualStringTree.Create(Self);
  VST5.Parent:= Self;

  VST5.OnAfterCellPaint := VST5AfterCellPaint;
  VST5.OnBeforeCellPaint := VST5BeforeCellPaint;
  VST5.OnBeforeItemErase := VST5BeforeItemErase;
  VST5.OnCreateEditor := VST5CreateEditor;
  VST5.OnFocusChanging := VST5FocusChanging;
  VST5.OnFreeNode := VST5FreeNode;
  VST5.OnGetText := VST5GetText;
  VST5.OnPaintText := VST5PaintText;
  VST5.OnInitNode := VST5InitNode;
  VST5.OnStateChange := VST5StateChange;

  VST5.Left := 6;
  VST5.Top := 6;
  VST5.Width := 491;
  VST5.Height := 428;
  VST5.Anchors := [akLeft, akTop, akRight, akBottom];
  VST5.BevelInner := bvNone;
  VST5.BevelOuter := bvRaised;
  VST5.BorderStyle := bsNone;
  {VST5.ClipboardFormats.Strings := (
    'CSV'
    'HTML Format'
    'Plain text'
    'Rich Text Format'
    ...
    'Unicode text'
    'Virtual Tree Data')   }
  VST5.Colors.BorderColor := clWindowText;
  VST5.Colors.HotColor := clBlack;
  VST5.DefaultNodeHeight := 19;
  VST5.DragMode := dmAutomatic;
  VST5.EditDelay := 300;
  VST5.Font.Charset := ANSI_CHARSET;
  VST5.Font.Color := clWindowText;
  VST5.Font.Height := -11;
  VST5.Font.Name := 'Microsoft Sans Serif';
  VST5.Font.Style := [];
  VST5.Header.AutoSizeIndex := 2;
  VST5.Header.Background := clBtnShadow;
  VST5.Header.Font.Charset := ANSI_CHARSET;
  VST5.Header.Font.Color := clWindowText;
  VST5.Header.Font.Height := -12;
  VST5.Header.Font.Name := 'Microsoft Sans Serif';
  VST5.Header.Font.Style := [];
  VST5.Header.Height := 20;
  VST5.Header.Options := [hoColumnResize, hoDblClickResize, hoDrag, hoShowImages, hoVisible];
  VST5.Header.Style := hsFlatButtons;
  //VST5.HintAnimation := hatFade;
  VST5.HintMode := hmTooltip;
  VST5.ParentFont := False;
  VST5.ParentShowHint := False;
  VST5.PopupMenu := PopupMenu;
  VST5.RootNodeCount := 100;
  VST5.ScrollBarOptions.AlwaysVisible := True;
  VST5.ShowHint := True;
  VST5.TabOrder := 0;
  VST5.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoTristateTracking];
  VST5.TreeOptions.MiscOptions := [toAcceptOLEDrop, toEditable, toGridExtensions, toInitOnSave, toToggleOnDblClick, toWheelPanning];
  VST5.TreeOptions.PaintOptions := [toHotTrack, toShowButtons, toShowDropmark, toShowHorzGridLines, toShowVertGridLines, toUseBlendedImages];
  VST5.TreeOptions.SelectionOptions := [toDisableDrawSelection, toExtendedFocus, toMiddleClickSelect, toMultiSelect, toRightClickSelect, toCenterScrollIntoView];
  VST5.WantTabs := True;

  col:= VST5.Header.Columns.Add;
  col.Color := clWindow;
  col.Options := [coParentBidiMode, coVisible, coFixed];
  col.Position := 0;
  col.Width := 20;
