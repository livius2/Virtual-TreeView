...
begin
  ...
  VirtualTrees.Utils.AlphaBlend(nil, Bitmap.Canvas, R, Point(0, 0), bmConstantAlphaAndColor, Alpha,
...
begin
  HeaderCustomDrawTree:= TVirtualStringTree.Create(Self);
  HeaderCustomDrawTree.Parent:= Self;

  HeaderCustomDrawTree.OnAdvancedHeaderDraw := HeaderCustomDrawTreeAdvancedHeaderDraw;
  HeaderCustomDrawTree.OnGetText := HeaderCustomDrawTreeGetText;
  HeaderCustomDrawTree.OnHeaderDrawQueryElements := HeaderCustomDrawTreeHeaderDrawQueryElements;
  HeaderCustomDrawTree.OnHeaderMouseDown := HeaderCustomDrawTreeHeaderMouseDown;
  HeaderCustomDrawTree.OnHeaderMouseUp := HeaderCustomDrawTreeHeaderMouseUp;
  HeaderCustomDrawTree.OnStateChange := HeaderCustomDrawTreeStateChange;

  HeaderCustomDrawTree.Left := 8;
  HeaderCustomDrawTree.Top := 10;
  HeaderCustomDrawTree.Width := 749;
  HeaderCustomDrawTree.Height := 301;
  HeaderCustomDrawTree.Anchors := [akLeft, akTop, akRight, akBottom];
  HeaderCustomDrawTree.BevelOuter := bvNone;
 { HeaderCustomDrawTree.ClipboardFormats.Strings := (
    'CSV'
    'HTML Format'
    'Plain text'
    'Rich Text Format'
    ...
    'Unicode text'
    'Virtual Tree Data')  }
  HeaderCustomDrawTree.Colors.BorderColor := clBackground;
  HeaderCustomDrawTree.Colors.FocusedSelectionColor := clSilver;
  HeaderCustomDrawTree.Colors.FocusedSelectionBorderColor := 10526880;
  HeaderCustomDrawTree.Colors.HotColor := clBlack;
  HeaderCustomDrawTree.Colors.UnfocusedSelectionBorderColor := clBtnShadow;
  HeaderCustomDrawTree.DefaultNodeHeight := 24;
  HeaderCustomDrawTree.EditDelay := 100;
  HeaderCustomDrawTree.Header.AutoSizeIndex := 0;
  HeaderCustomDrawTree.Header.Background := clBtnShadow;
  HeaderCustomDrawTree.Header.Font.Charset := DEFAULT_CHARSET;
  HeaderCustomDrawTree.Header.Font.Color := clWindowText;
  HeaderCustomDrawTree.Header.Font.Height := -11;
  HeaderCustomDrawTree.Header.Font.Name := 'Tahoma';
  HeaderCustomDrawTree.Header.Font.Style := [];
  HeaderCustomDrawTree.Header.Height := 110;
  HeaderCustomDrawTree.Header.Images := HeaderImages;
  HeaderCustomDrawTree.Header.Options := [hoColumnResize, hoDblClickResize, hoDrag, hoHotTrack, hoOwnerDraw, hoShowHint, hoShowImages, hoShowSortGlyphs, hoVisible];
  HeaderCustomDrawTree.Header.SortColumn := 0;
  HeaderCustomDrawTree.Header.Style := hsFlatButtons;
  HeaderCustomDrawTree.HintMode := hmTooltip;
  HeaderCustomDrawTree.HotCursor := crHandPoint;
  HeaderCustomDrawTree.LineMode := lmBands;
  HeaderCustomDrawTree.LineStyle := lsSolid;
  HeaderCustomDrawTree.ParentShowHint := False;
  HeaderCustomDrawTree.RootNodeCount := 20;
  HeaderCustomDrawTree.SelectionCurveRadius := 5;
  HeaderCustomDrawTree.ShowHint := True;
  HeaderCustomDrawTree.TabOrder := 0;
  HeaderCustomDrawTree.TreeOptions.AnimationOptions := [toAnimatedToggle];
  HeaderCustomDrawTree.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoScrollOnExpand, toAutoTristateTracking, toAutoDeleteMovedNodes];
  HeaderCustomDrawTree.TreeOptions.MiscOptions := [toCheckSupport, toEditable, toFullRepaintOnResize, toGridExtensions, toInitOnSave, toWheelPanning];
  HeaderCustomDrawTree.TreeOptions.PaintOptions := [toHideFocusRect, toShowHorzGridLines, toShowVertGridLines, toThemeAware, toUseBlendedImages];
  HeaderCustomDrawTree.TreeOptions.SelectionOptions := [toDisableDrawSelection, toFullRowSelect];

  col:= HeaderCustomDrawTree.Header.Columns.Add;
  col.BiDiMode := bdLeftToRight;
  col.Color := clWindow;
  col.ImageIndex := 1;
  col.Options := [coDraggable, coEnabled, coResizable, coShowDropMark, coVisible];
  col.Position := 0;
  col.Spacing := -1;
  col.Width := 200;
  col.Text := 'Default drawing';
  col.Hint := 'This column is drawn entirely by the tree.';
  ...
  col.Color := clWindow;
  col.ImageIndex := 2;
  col.Layout := blGlyphTop;
  col.Options := [coAllowClick, coDraggable, coEnabled, coResizable, coShowDropMark, coVisible];
  col.Position := 1;
  col.Spacing := -10;
  col.Style := vsOwnerDraw;
  col.Width := 230;
  col.Text := 'Custom background';
  col.Hint := 'Only the background is customized.';
  ...
  col.Style := vsOwnerDraw;
  col.Width := 350;
  col.Hint := 'Background and text in this column are customized.';
