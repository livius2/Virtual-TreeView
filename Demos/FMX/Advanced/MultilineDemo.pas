...
begin
  MLTree:= TVirtualStringTree.Create(Self);
  MLTree.Parent:= Panel1;

  MLTree.Left := 96;
  MLTree.Top := 8;
  MLTree.Width := 533;
  MLTree.Height := 337;
  MLTree.Anchors := [akLeft, akTop, akRight, akBottom];
  MLTree.BorderStyle := bsNone;
  {MLTree.ClipboardFormats.Strings := (
    'CSV'
    'HTML Format'
    'Plain text'
    'Rich Text Format'
    ...
    'Unicode text'
    'Virtual Tree Data')}
  MLTree.Colors.SelectionRectangleBlendColor := 10539203;
  MLTree.DefaultNodeHeight := 130;
  MLTree.DragMode := dmAutomatic;
  MLTree.Header.AutoSizeIndex := 0;
  MLTree.Header.Font.Charset := DEFAULT_CHARSET;
  MLTree.Header.Font.Color := clWindowText;
  MLTree.Header.Font.Height := -11;
  MLTree.Header.Font.Name := 'Tahoma';
  MLTree.Header.Font.Style := [];
  MLTree.Header.Options := [hoAutoResize, hoColumnResize, hoDrag];
  //MLTree.HintAnimation := hatNone;
  MLTree.HintMode := hmTooltip;
  MLTree.LineMode := lmBands;
  MLTree.NodeAlignment := naFromTop;
  MLTree.NodeDataSize := 4;
  MLTree.ParentShowHint := False;
  MLTree.RootNodeCount := 30;
  MLTree.SelectionCurveRadius := 10;
  MLTree.ShowHint := True;
  MLTree.TabOrder := 0;
  MLTree.TextMargin := 5;
  MLTree.TreeOptions.AutoOptions := [toAutoDropExpand, toAutoScroll, toAutoScrollOnExpand, toAutoSpanColumns, toAutoTristateTracking, toAutoDeleteMovedNodes];
  MLTree.TreeOptions.MiscOptions := [toEditable, toInitOnSave, toReportMode, toToggleOnDblClick, toWheelPanning];
  MLTree.TreeOptions.PaintOptions := [toHideFocusRect, toShowDropmark, toShowTreeLines, toThemeAware, toUseBlendedImages];
  MLTree.TreeOptions.SelectionOptions := [toMultiSelect];
  MLTree.OnEditing := MLTreeEditing;
  MLTree.OnGetText := MLTreeGetText;
  MLTree.OnPaintText := MLTreePaintText;
  MLTree.OnInitNode := MLTreeInitNode;
  MLTree.OnMeasureItem := MLTreeMeasureItem;
  MLTree.OnStateChange := MLTreeStateChange;
  col:= MLTree.Header.Columns.Add;
  col.Position := 1;
  col.Width := 466;
  col:= MLTree.Header.Columns.Add;
  col.Position := 0;
